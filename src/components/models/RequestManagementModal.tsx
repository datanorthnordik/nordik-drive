...
function RequestManagementModal(props: RequestManagementModalProps) {
    ...
    const { data: roles, loading, error, fetchData } = useFetch("http://127.0.0.1:8080/role", "GET", false)
    const { data: files, loading: cloading, error: cerror, fetchData: cFetchData } = useFetch("http://127.0.0.1:8080/file", "GET", true)
    const { data: updateRequest, loading: uloading, fetchData: ufetchData } = useFetch("http://127.0.0.1:8080/requests/update", "PUT", false)
