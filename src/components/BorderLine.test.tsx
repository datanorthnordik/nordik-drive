...
import BorderLine from "./BorderLine"; // adjust if needed
import { color_border } from "../constants/colors"; // adjust if needed

const getCss = () => {
  const sheet = new ServerStyleSheet();
  try {
    renderToString(sheet.collectStyles(<BorderLine />));
    return sheet.getStyleTags(); // includes CSS in <style> tag
  } finally {
    sheet.seal();
  }
};
...
describe("BorderLine", () => {
  test("injects expected base CSS", () => {
    ...
    // whitespace-tolerant gradient assertion
  ...
  test("injects expected ::before styles and OR label", () => {
    ...
    // translate has spaces in output => allow optional whitespace
