...
const PhotoViewerModal: React.FC<PhotoViewerModalProps> = ({
  ...
  const inferredRequestId = useMemo(() => {
    const p = (photos || []).find(
      (x) => typeof x?.request_id === "number" || typeof x?.requestId === "number"
    );
    return (p?.request_id ?? p?.requestId ?? null) as number | null;
  }, [photos]);
  ...
  const canDownloadAll = Boolean(inferredRequestId);

  const handleDownloadAllPhotos = useCallback(async () => {
    if (!canDownloadAll || zipLoading) return;

    zipNonceRef.current += 1;
    zipNameRef.current = buildZipName(`photos_request_${inferredRequestId}`);
    ...
    const body = {
      ...
      request_id: inferredRequestId,
    };

    await fetchZip(body as any, undefined, false, { responseType: "blob" });
  }, [canDownloadAll, zipLoading, fetchZip, inferredRequestId]);
      ...
      >
        <Box sx={{ minWidth: 0, flex: 1 }}>
          ...
          <Typography variant="caption" sx={{ color: color_text_light, fontWeight: 700 }}>
            ...
            {inferredRequestId ? ` • Request #${inferredRequestId}` : ""}
          </Typography>
        </Box>
            ...
            >
              ...
              <Tooltip
                title={
                  canDownloadAll
                    ? "Download all photos (ZIP)"
                    : "Request ID not found (add request_id in photos)"
                }
              >
                <span>
                  <IconButton
