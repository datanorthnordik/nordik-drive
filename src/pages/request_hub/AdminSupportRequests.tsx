...
export default function AdminSupportRequests() {
  ...
  const saveManagement = async () => {
    ...
    if (
      ...
      toast.error("Choose the team and enter at least one forwarding email before marking this In Progress.");
      return;
    }
  ...
  return (
    ...
    >
      ...
      >
        <Box
          sx={{
            ...
            background:
              "linear-gradient(120deg, rgba(238,245,255,0.98) 0%, rgba(255,255,255,1) 62%)",
            display: "flex",
        ...
        <Box sx={{ p: { xs: 1.25, md: 2.25 }, background: color_background }}>
          <Stack direction={{ xs: "column", lg: "row" }} spacing={1.25} sx={{ mb: 1.75 }}>
            {[
              { status: SUPPORT_REQUEST_STATUS.OPEN, icon: <MarkEmailUnreadRoundedIcon />, helper: "Needs triage" },
              { status: SUPPORT_REQUEST_STATUS.IN_PROGRESS, icon: <SupportAgentRoundedIcon />, helper: "With a team" },
              { status: SUPPORT_REQUEST_STATUS.CLOSED, icon: <AssignmentTurnedInRoundedIcon />, helper: "Completed" },
              ...
              return (
                <Button
                  ...
                  sx={{
                    ...
                    textAlign: "left",
                    textTransform: "none",
                    background: selected ? chip.sx.backgroundColor : color_white,
                    border: `1px solid ${selected ? chip.sx.border : color_border}`,
                    color: color_text_primary,
                    "&:hover": { background: chip.sx.backgroundColor },
                  }}
                >
                  <Box sx={{ color: chip.accent, mr: 1.2, display: "grid", placeItems: "center" }}>{icon}</Box>
                  <Box>
                    <Typography sx={{ fontWeight: 950, fontSize: 15 }}>{getSupportRequestStatusLabel(status)}</Typography>
                    <Typography sx={{ fontWeight: 700, fontSize: 12.5, color: color_text_light }}>{helper} · {statusCounts[status]}</Typography>
                  </Box>
                </Button>
              );
            })}
                  ...
                  >
                    ...
                    >
                      <Box sx={{ minWidth: 0, flex: 1 }}>
                        ...
                        <Typography sx={{ color: color_text_secondary, fontWeight: 800, fontSize: 13.2, mt: 0.25 }}>
                          {request.requester_name} · {request.requester_email}
                        </Typography>
                        <Typography sx={{ color: color_text_light, fontWeight: 700, fontSize: 12.5, mt: 0.25 }}>
                          #{request.id} · {getSupportRequestTypeLabel(request.request_type)} · {formatSupportRequestDate(request.created_at)}
                        </Typography>
                      </Box>
                      <Stack direction="row" spacing={0.75} alignItems="center">
                        <Chip label={status.label} size="small" sx={{ borderRadius: "999px", fontWeight: 950, ...status.sx }} />
                        <Button
                    ...
                    {request.assigned_team ? (
                      <Typography sx={{ mt: 1, color: color_text_light, fontWeight: 850, fontSize: 12.8 }}>
                        Assigned to {request.assigned_team} · {request.status === SUPPORT_REQUEST_STATUS.CLOSED ? "Closed" : "Forwarded"} {formatSupportRequestDate(request.closed_at || request.assigned_at)}
                      </Typography>
                    ) : null}
                  </Paper>
                );
      ...
      >
        {activeRequest && draft ? (
          <>
            <DialogTitle sx={{ px: { xs: 1.8, md: 2.5 }, py: 1.6, background: color_white_smoke, borderBottom: `1px solid ${color_border}` }}>
              <Typography sx={{ color: color_text_primary, fontWeight: 950, fontSize: 18 }}>
                Manage Support Request #{activeRequest.id}
              </Typography>
              <Typography sx={{ color: color_text_light, fontWeight: 700, fontSize: 13, mt: 0.3 }}>
                Updating the request emails the user. Forwarding also emails the selected team.
              </Typography>
            </DialogTitle>
            <DialogContent sx={{ px: { xs: 1.8, md: 2.5 }, py: 2.2 }}>
              <Stack spacing={1.55}>
                <Box sx={{ p: 1.35, borderRadius: "15px", border: `1px solid ${color_border}`, background: color_white }}>
                  <Typography sx={{ color: color_text_primary, fontWeight: 950, fontSize: 16 }}>
                    {activeRequest.subject}
                  </Typography>
                  <Typography sx={{ color: color_text_secondary, fontWeight: 800, fontSize: 13.2, mt: 0.35 }}>
                    {activeRequest.requester_name} · {activeRequest.requester_email}
                  </Typography>
                  <Typography sx={{ color: color_text_light, fontWeight: 700, fontSize: 12.6, mt: 0.25 }}>
                    {getSupportRequestTypeLabel(activeRequest.request_type)} · Submitted {formatSupportRequestDate(activeRequest.created_at)}
                  </Typography>
                  <Typography sx={{ color: color_text_secondary, fontWeight: 700, fontSize: 13.5, mt: 1.05, lineHeight: 1.65, whiteSpace: "pre-wrap" }}>
                  ...
                  {activeRequest.screenshot_url ? (
                    <Button
                      ...
                      sx={{ mt: 1, textTransform: "none", fontWeight: 900, color: color_secondary_dark }}
                    >
                      View screenshot{activeRequest.screenshot_file_name ? `: ${activeRequest.screenshot_file_name}` : ""}
                    </Button>
                  ) : null}
                </Box>
                ...
                <TextField
                  ...
                  value={draft.status}
                  onChange={(event) => updateDraft("status", event.target.value as SupportRequestStatus)}
                  fullWidth
                  size="small"
                ...
                <Box
                  sx={{
                    ...
                    background: draft.status === SUPPORT_REQUEST_STATUS.IN_PROGRESS ? "rgba(238,245,255,0.75)" : color_white,
                  }}
                >
                  <Typography sx={{ color: color_text_primary, fontWeight: 950, fontSize: 14.5 }}>
                    Forward to associated team
                  </Typography>
                  <Typography sx={{ color: color_text_light, fontWeight: 700, fontSize: 12.8, mt: 0.25, mb: 1.15 }}>
                  ...
                  <Stack direction={{ xs: "column", sm: "row" }} spacing={1.15}>
                    ...
                    <TextField
                      ...
                      onChange={(event) => updateDraft("assignedTeamRecipients", event.target.value)}
                      fullWidth
                      size="small"
            ...
            <DialogActions sx={{ px: { xs: 1.8, md: 2.5 }, py: 1.5, borderTop: `1px solid ${color_border}`, background: color_white }}>
              <Button
                onClick={closeManager}
                disabled={saving}
                sx={{ textTransform: "none", color: color_text_secondary, fontWeight: 900 }}
              >
                Cancel
              </Button>
              <Button
              ...
              >
                {saving ? "Saving..." : draft.status === SUPPORT_REQUEST_STATUS.IN_PROGRESS ? "Forward & notify" : "Save & notify"}
              </Button>
            </DialogActions>
          </>
        ) : null}
